            ...
            exports.queue.process(options.queue, options.concurrency, function(job, done) {
                ...
                if (_id) {
                    ...
                    async.waterfall([
                        ...
                        function findPushNotificationById(next) {
                            PushNotification.findById(_id, next);
                        },

                        function ensurePushNotificationExists(_pushNotification, next) {
                            if (!_pushNotification) {
                                next(new Error('PushNotification with id ' + _id + ' does not exists'));
                            } else {
                                next(null, _pushNotification);
                            }
                        },

                        function sendEmail(_pushNotification, next) {
